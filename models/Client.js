  ...
  },
  {
    ...
    classMethods: {
      ...
      login: function(email,password){
        ...
        var seller
        return Seller.find({where: {email: email}})
          .then(function(result){
            if(!result) throw new Error('No seller found')
            if(!result.active) throw new Error('Seller inactive')
            //globalize seller
            seller = result
            //verify password
            return bcrypt.compareAsync(password,seller.password)
          })
          .then(function(match){
            if(!match) throw new Error('Invalid password')
            return seller.updateAttributes({dateSeen: now})
          })
          .then(function(){
          ...
          .catch(function(err){
            if(seller) seller.updateAttributes({dateFail: now})
            throw err
          })
      }
    }
